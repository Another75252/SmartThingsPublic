 ...
 *
 *  This DTH was originally developed by SmartThings. I only modified it to allow enable/disable of the arrival sensor.
 *
 */
metadata {
	definition (name: "Enhanced Arrival Sensor", namespace: "smartthings", author: "ady624") {
		capability "Tone"
		capability "Actuator"
	...
	tiles {
		...
		standardTile("enabled", "device.enabled") {
			state "off", label:'OFF', action:"toggle", icon: "st.switches.light.off", backgroundColor:"#cccccc"
			state "on", label:'ON', action:"toggle", icon: "st.switches.light.on", backgroundColor:"#ffffff"
		}
		...
		main "presence"
		details(["presence", "battery", "lqi", "enabled", "beep"])
	}
}
...
private Map parsePresenceMessage(String description) {
	//if device is not in use, we prevent updating the presence
	if (device.currentValue("enabled") != "on") return null
...
def toggle() {
	if (device.currentValue("enabled") == "on") {
    	disable()        
    } else {
    	enable()
    }
...
def enable() {
	sendEvent(name: "enabled", value: "on", isStateChange: true)
}
